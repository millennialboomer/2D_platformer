...
def get_background(name):
    image = py.image.load(join("assets","Background", name))
...
def handle_move(player,objects):
    ...
    if keys[py.K_a] and not collide_left:
...
def main(window):
    ...
    fire.on()
    floor = [Block(i * block_size, HEIGHT - block_size, block_size) for i in range(
    ...
    while run:
        ...
        for event in py.event.get():
            ...
            if event.type == py.KEYDOWN:
                if event.key == py.K_w and player.jump_count < 2:
                    player.jump()
        ...
        if ((player.rect.right - offset_x >= WIDTH - scroll_area_width) and player.x_vel > 0) or (
                (player.rect.left - offset_x <= scroll_area_width) and player.x_vel < 0): #THIS  IF STATEMENT MAY CAUSE ISSUES
